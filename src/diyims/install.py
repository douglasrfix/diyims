...
def install_main(call_stack, drive_letter, force_install):
    ...
    # FIXME: create stand alone version that can be run at update time
    ...
    if drive_letter != "Default":
        if drive_letter != Path(install_template_dict["db_path"]).drive:
            ...
            install_template_dict["update_path"] = Path(drive_letter + "/").joinpath(
                "diyims", "Update"
            )
    ...
    update_path = install_template_dict["update_path"]
    ...
    if override_drive != "True":
        ...
        update_path.mkdir(mode=0o777, parents=True, exist_ok=True)
    ...
    parser["Paths"]["update_path"] = str(update_path)
