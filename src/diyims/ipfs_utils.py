...
def test_ipfs_version(call_stack, response_dictionary):
    # import json
    import os
    # import requests
    ...
    from diyims.sqlmodels import IPFS_Agents
    from sqlalchemy.exc import NoResultFound
    from diyims.error_classes import UnSupportedIPFSVersionError
    from diyims.database_utils import sync_ipfs_agent_table

    sync_ipfs_agent_table()
    # url_dict = get_url_dict()
    call_stack = call_stack + "test_ipfs_version"
    ...
    statement = select(IPFS_Agents).where(
        IPFS_Agents.agent_id == response_dictionary["AgentVersion"]
    )
    ...
    with Session(engine) as session:
        ...
        if match_count == 0:
            raise (UnSupportedIPFSVersionError(response_dictionary["AgentVersion"]))

    return response_dictionary["AgentVersion"]
...
def wait_on_ipfs(call_stack):
    from time import sleep
    from diyims.requests_utils import execute_request
    from diyims.config_utils import get_ipfs_config_dict
    from diyims.error_classes import CommunicationsError, UnSupportedIPFSVersionError
    ...
    while error_found:
        try:
            response, status_code, response_dict = execute_request(url_key="id")
            error_found = False
        except CommunicationsError as e:
            i += 1
            sleep(int(ipfs_config_dict["connect_retry_delay"]))
            if i > (int(ipfs_config_dict["connect_retries"])):
                raise CommunicationsError(e)
    try:
        test_ipfs_version(call_stack, response_dict)
