...
def get_path_dict():
    ...
    path_dict["update_path"] = Path(parser["Paths"]["update_path"])
    path_dict["dist_path"] = Path(parser["Paths"]["dist_path"])
    path_dict["generic_path"] = Path(parser["Paths"]["generic_path"])
    return path_dict
...
def get_linux_template_dict():
    ...
    xdg_data_home = Path(xdg_home).joinpath(
        "local", "diyims", "share"
    )  # removed . to ease debugging
    ...
    xdg_update_home = Path(xdg_home).joinpath("local", "diyims", "update")
    xdg_dist_home = Path(xdg_home).joinpath("local", "diyims", "dist")
    xdg_generic_home = Path(xdg_home).joinpath("local", "diyims", "generic")
    ...
    template_path_dict["update_path"] = xdg_update_home
    template_path_dict["dist_path"] = xdg_dist_home
    template_path_dict["generic_path"] = xdg_generic_home
...
def get_win32_template_dict():
    ...
    xdg_update_home = Path(xdg_home).joinpath("AppData", Roaming, "diyims", "Update")
    xdg_dist_home = Path(xdg_home).joinpath("AppData", Roaming, "diyims", "Dist")
    xdg_generic_home = Path(xdg_home).joinpath("AppData", Roaming, "diyims", "Generic")
