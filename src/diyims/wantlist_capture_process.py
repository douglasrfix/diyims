...
from diyims.general_utils import get_DTS, set_self, shutdown_query
...
def filter_wantlist(
    ...
    doc string
    """
    from diyims.sqlmodels import Peer_Table, Want_List_Table

    call_stack = call_stack + ":filter_wantlist"
    current_DT = datetime.now(timezone.utc)  # noqa: UP017
    ...
    with Session(engine) as session:
        ...
        for want_list_item in results:
            line_list.append(want_list_item)  # noqa: PERF402
                    ...
                    if provider_peer_row_CID != "null":
                        ...
                        if test_peer_ID != provider_peer_ID:
                            ...
                            with Session(engine) as session:
                                results = session.exec(statement)
                                peer_table_row = results.one()
                            # here we see if the beacon info matched the info in the cid
